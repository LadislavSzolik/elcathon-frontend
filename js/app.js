                ...
                if ($scope.listOfTasks[i + 1] != null) {
                    ...
                    for (var i = 0; i < SharedBackend.listOfHowTo.length; i++) {
                        if (SharedBackend.currID == SharedBackend.listOfHowTo[i].ID) {
                            SharedBackend.listOfHowTo[i].inprogress = false;
                        }
                    }
    ...
    $scope.goOverview = function(selectedId) {
        ...
        for (var i = 0; i < SharedBackend.listOfHowTo.length; i++) {
            if (selectedId == SharedBackend.listOfHowTo[i].ID) {
                SharedBackend.currID = selectedId;
            }
        }
...
ctnApp.controller('myProgressCtrl', ['$scope', '$window', 'SharedBackend', function($scope, $window, SharedBackend) {

    $scope.taskInProgress = [];
    for (var i = 0; i < SharedBackend.listOfHowTo.length; i++) {
        if (SharedBackend.listOfHowTo[i].inprogress == true) {
            $scope.taskInProgress.push(SharedBackend.listOfHowTo[i]);
        }
    }
...
ctnApp.controller('overviewCtrl', ['$scope', '$window', 'SharedBackend', function($scope, $window, SharedBackend) {
    ...
    $scope.start = function() {
        ...
        for (var i = 0; i < SharedBackend.listOfHowTo.length; i++) {
            if (SharedBackend.currID == SharedBackend.listOfHowTo[i].ID) {
                SharedBackend.listOfHowTo[i].inprogress = true;
            }
        }
